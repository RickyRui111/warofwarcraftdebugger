    ...
    ui(new Ui::compare)
{
    ...
    ui->cTextLines->hide();
    ui->wTextLines->hide();
    ui->aclabel ->setPixmap(QPixmap(":/images/QTpictures/Accepted.png"));
    ui->aclabel ->setScaledContents(true);
    ui->aclabel->hide();
}
...
void compare::on_confirmbt_clicked()
{
    QFile wrongTextFile("wrongTextFile.txt");
            ...
            ui->wTextLines->setText(QString("第一处错误行数：").arg(wlines));
            //ui->difLines->setText(QString("错误行数：").arg(wlines));//加上while循环后似乎解决了
            correctTextFile.close();
            wrongTextFile.close();
